...
//	}
	...
	@Override
	public Set<ProjectDto> getAllProjects(Long companyId, Long teamId, boolean includeInactive) {
		...
		// Apply the filter for inactive projects only if includeInactive is false.
		if (!includeInactive) {
			// This will remove any project where isActive() returns false
			filteredProjects.removeIf(project -> !project.isActive());
		}
